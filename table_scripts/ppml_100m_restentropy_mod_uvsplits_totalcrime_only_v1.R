...
LOGFILE <- file.path(OUT_TABLE, "ppml_100m_restentropy_mod_uvsplits_totalcrime_only_v1.log")
...
# Open log
sink(LOGFILE)
cat("Starting PPML analysis: Restaurant density and cuisine entropy moderation on TOTAL CRIME ONLY\n")
cat("Specification: Same as ppml_100m_restentropy_mod_uvsplits_alluv_totalcrime_housentl_v2.csv\n")
cat("Focus: Total crime (crime_count) with triple interaction z_us4 * post * moderator\n")
cat(Sys.time(), "\n\n")
...
run_ppml_triple <- function(data, yvar, moderator_var, mod_label) {
  ...
  if (nrow(dt_clean) == 0) {
    cat(sprintf("    WARNING: No valid observations for %s\n", mod_label))
    return(NULL)
  }
  ...
  result <- tryCatch({
    ...
    if (is.na(b) || is.na(se) || se == 0) {
      cat(sprintf("  WARNING: Invalid coefficient or SE for %s\n", mod_label))
      return(NULL)
    }
    ...
    cat(sprintf("    %s: Coef=%.4f, SE=%.4f, p=%.6f, N=%d\n",
                mod_label, b, se, p_val, summ$nobs))
...
# =========================================================
# Run regressions for total crime only
# =========================================================
cat("\n=== Running PPML Regressions for Total Crime ===\n")
...
# Export CSV
out_csv <- file.path(OUT_TABLE, "ppml_100m_restentropy_mod_uvsplits_totalcrime_only_v1.csv")
...
cat("\nDone: PPML moderation analysis for total crime completed.\n")
sink()
